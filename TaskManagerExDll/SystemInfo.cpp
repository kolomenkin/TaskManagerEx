...
void SystemModuleInformation::GetModuleListForProcess( DWORD processID )
{
	...
	for ( i = 0; i < cbNeeded / sizeof( HMODULE ); i++ )
	{
		...
		if ( (*m_GetModuleFileNameEx)( hProcess, hModules[i], moduleInfo.FullPath, _MAX_PATH ) &&
		     (*m_GetModuleBaseName)( hProcess, hModules[i], moduleInfo.Name, _MAX_PATH ))
...
BOOL SystemModuleInformation::Refresh()
{
   ...
   m_GetModuleBaseName = (PGetModuleFileNameEx)GetProcAddress( hDll, 
#ifdef UNICODE
								"GetModuleBaseNameW" );
#else
								"GetModuleBaseNameA" );
	...
	if ( m_GetModuleFileNameEx == NULL || m_EnumProcessModules == NULL || m_GetModuleBaseName == NULL )
	{
		rc = FALSE;
		goto cleanup;
	}
